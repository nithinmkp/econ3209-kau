...
resid_df = pd.DataFrame({"resid_t": ols.resid[1:], "resid_lag": ols.resid[:-1]})
rho_hat = smf.ols("resid_t ~ resid_lag - 1", data=resid_df).fit().params["resid_lag"]
...
trans['rain_star'] = trans['rain'] - rho_hat * trans['rain'].shift(1)
co = smf.ols('price_star ~ trend_star + rain_star', data=trans.dropna()).fit()
print('rho_hat =', round(rho_hat, 4))
print('OLS trend coefficient =', round(ols.params['trend'], 4))
print('Cochrane-Orcutt trend coefficient =', round(co.params['trend_star'], 4))
```
