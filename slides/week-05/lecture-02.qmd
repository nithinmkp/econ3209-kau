...
if hasattr(sns, "distplot"):
    sns.distplot(loan_df["loan_size"], kde=True, ax=ax, color="#7AB87A")
else:
    sns.histplot(loan_df["loan_size"], kde=True, ax=ax, color="#7AB87A")
...
fig, ax = plt.subplots(figsize=(7, 4))
sns.boxplot(data=yield_df, x="crop", y="yield_kg", ax=ax, palette=["#7AB87A", "#DAB300"])
...
fig, ax = plt.subplots(figsize=(7, 4))
sns.barplot(data=df, x="district", y="deposit_crore", color="#7AB87A", ax=ax)
...
df = pd.DataFrame({
    'crop':  ['Rice'] * 5 + ['Wheat'] * 5 + ['Coconut'] * 5,
    'price': [35, 38, 36, 34, 37, 22, 24, 23, 25, 21, 28, 30, 27, 31, 29]
})

sns.boxplot(data=df, x='crop', y='price', palette='Greens')
plt.title('Price Distribution by Crop (₹/kg)')
plt.ylabel('Price (₹/kg)')
plt.tight_layout()
plt.show()
```
