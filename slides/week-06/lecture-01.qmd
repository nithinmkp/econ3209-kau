...
## MOSPI: India's Official Micro-data Source

- **MOSPI** (Ministry of Statistics & Programme Implementation) is India's apex statistical body.
- Publishes large-scale unit-level surveys used in empirical economics research.

| Dataset | Coverage | Key Variables |
|---------|----------|---------------|
| **NSS** (National Sample Survey) | Households, consumption, employment | Expenditure, MPCE, employment status |
| **PLFS** (Periodic Labour Force Survey) | Workers 15+ | WPR, LFPR, wages, industry |
| **ASI** (Annual Survey of Industries) | Formal manufacturing plants | Output, employment, capital |
| **NSSO Debt & Investment** | Rural/urban households | Assets, liabilities, interest rates |

::: {.highlight-box}
Unit-level data → one row per household or worker → essential for microeconometrics.
:::

---

## Accessing MOSPI Data in Python

```{pyodide-python}
import pandas as pd
from io import StringIO

# Simulated PLFS-style unit data (actual data downloaded from mospi.gov.in)
# Variables: state, sector (rural/urban), usual_activity, weekly_wage
plfs_text = """state,sector,usual_activity,weekly_wage
Kerala,Rural,Self-employed,3200
Kerala,Urban,Regular wage,8500
Tamil Nadu,Rural,Casual labour,1800
Karnataka,Urban,Regular wage,9200
Kerala,Rural,Regular wage,6100
Kerala,Urban,Self-employed,7400
"""

plfs = pd.read_csv(StringIO(plfs_text))

# State-level wage summary (typical PLFS analysis)
summary = plfs.groupby(['state', 'sector'])['weekly_wage'].agg(['mean', 'count'])
summary.columns = ['avg_wage', 'n_workers']
print(summary.round(0))
```

::: {.highlight-box}
**Actual download:** [mospi.gov.in](https://mospi.gov.in) → Surveys → Unit Level Data → Register (free)
:::

---

## MOSPI eSankhyiki: The Data Portal

- **eSankhyiki** ([esankhyiki.mospi.gov.in](https://esankhyiki.mospi.gov.in)) is MOSPI's open-access portal.
- Provides **aggregate tables** (not unit-level) downloadable as CSV/Excel — no registration needed.

**What you can get directly:**

| Indicator | Frequency | Format |
|-----------|-----------|--------|
| State Domestic Product (GSDP) | Annual | Excel |
| CPI by state and commodity group | Monthly | CSV |
| Index of Industrial Production (IIP) | Monthly | Excel |
| Employment (Quarterly Bulletin) | Quarterly | PDF/Excel |

```{pyodide-python}
import pandas as pd
from io import StringIO

# Simulated eSankhyiki-style GSDP download (CSV from portal)
gsdp_text = """state,year,gsdp_cr,growth_pct
Kerala,2021,920340,8.1
Kerala,2022,1012400,10.0
Tamil Nadu,2021,1985600,9.3
Tamil Nadu,2022,2198000,10.7
Karnataka,2021,2102800,9.8
Karnataka,2022,2334500,11.0
"""

gsdp = pd.read_csv(StringIO(gsdp_text))
print(gsdp.pivot(index='state', columns='year', values='growth_pct'))
```

::: {.highlight-box}
eSankhyiki = ready-to-use aggregate tables · Unit Data Portal = raw micro-data for research
:::
...
- ✅ **MOSPI Unit Data** (NSS, PLFS, ASI) provides household and firm-level micro-data for India.
- ✅ **eSankhyiki** portal gives free access to aggregate tables (GSDP, CPI, IIP) in CSV/Excel.
