...
compare = pd.DataFrame({
    'RMSE_on_income': [
        np.sqrt(np.mean((df['income'] - np.exp(log_model.fittedvalues)) ** 2)),
        np.sqrt(np.mean((df['income'] - np.exp(loglin.fittedvalues)) ** 2)),
        np.sqrt(np.mean((df['income'] - linlog.fittedvalues) ** 2)),
    ]
