...
df = pd.DataFrame({'fertilizer': fertilizer,
                   'rainfall':   rainfall,
                   'irrigation': irrigation})

# True DGP: yield depends on all three regressors
df['yield_'] = (20
                + 0.25 * df['fertilizer']
                + 0.01 * df['rainfall']
                + 5.0  * df['irrigation']
...
# Matrix OLS
X = np.column_stack([np.ones(n), df['fertilizer'], df['rainfall'], df['irrigation']])
Y = df['yield_'].values
beta_matrix = np.linalg.solve(X.T @ X, X.T @ Y)
print("Matrix β̂:", beta_matrix.round(4))

# statsmodels — should match matrix result
